<template>
  <div class="app-container">
    <div class="filter-container">
      <el-select v-model="listQuery.delFlag" placeholder="Status" clearable class="filter-item" style="width: 130px">
...
export default {
  ...
  methods: {
    setPageTitle() {
      const title = '服务灰度'
      document.title = `${title} - ${this.listQuery.resource}`
    },
    setTagsViewTitle() {
      const title = '服务灰度'
      const route = Object.assign({}, this.tempRoute, { title: `${title}-${this.listQuery.resource}` })
    ...
    resetTemp() {
      this.temp = {
        ...
        moduleId: this.listQuery.resource,
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['Service Id', 'Policy Id', 'Policy Alias', 'Operator', 'Operate Time']
        const filterVal = ['resource', 'policyId', 'policyAlias', 'operator', 'operateTime']
