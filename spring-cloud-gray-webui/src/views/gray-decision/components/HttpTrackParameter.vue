<template>
  <div class="HttpTrackParameter">
    <el-form
      ref="HttpTrackParameter"
      :rules="rules"
      :model="infos"
      :inline="true"
    >
...
export default {
  ...
  methods: {
    ...
    clear() {
      this.$refs.HttpTrackParameter.clearValidate()
    },
    check() {
      let flag = false
      this.$refs.HttpTrackParameter.validate((valid) => {
        flag = valid
      })
      return flag
    }
...
<style >
.HttpTrackParameter .selectBox .el-form-item{
    margin-right: 0;
margin-left: 20px;
}
.HttpTrackParameter .selectBox .el-form-item:first-child{
    margin: 0;
}
.HttpTrackParameter .infosBox{
    width: 100%;
}
.HttpTrackParameter .infosBox .el-form-item__content{
    width:80%;
}
</style>
