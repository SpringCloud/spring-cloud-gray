...
export default {
  ...
  methods: {
    ...
    clear() {
      this.$refs.TrackAttributes.clearValidate()
    },
    check() {
      let flag = false
      this.$refs.TrackAttributes.validate((valid) => {
        flag = valid
      })
      return flag
    }
...
<style >
.TrackAttributes .selectBox .el-form-item{
    margin-right: 0;
margin-left: 20px;
}
.TrackAttributes .selectBox .el-form-item:first-child{
    margin: 0;
}
.TrackAttributes .infosBox{
    width: 100%;
}
.TrackAttributes .infosBox .el-form-item__content{
    width:80%;
}
</style>
